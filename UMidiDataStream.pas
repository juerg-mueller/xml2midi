...
type
  ...
  public
    ...
    function MakeEventArray(EventArray: TEventArray): boolean;
  end;
...
begin
  ...
  begin
//    if RaiseExcept then
//      raise Exception.Create('Restliche Dateigrösse kleiner als die angegebene Chunkgröße!');
  end;
  result := true;
end;
...
function TMidiDataStream.MakeEventArray(EventArray: TEventArray): boolean;
var
  iEvent: integer;
  iTrack: integer;
  Event: TMidiEvent;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not TEventArray.HasSound(EventArray.TrackArr[iTrack-1]) then
        begin
      ...
      begin
        ...
        if (event.d2 > 0) { and
           ((event.command <> $ff) or not (Event.d1 in [5, 51])) } then
        begin
          Event.var_len := ReadVariableLen;
          inc(EventArray.TrackArr[iTrack-1][iEvent-1].var_len, Event.var_len);
          inc(offset, Event.var_len);
    ...
    {        5,   // Lyrics
            51:  // set tempo
               ...
               EventArray.DetailHeader.SetParams(Event, Bytes);
          end;
        end
      end else
      if event.Event in [8..14] then
...
begin
  ...
  begin
    ...
    begin
      ...
      WriteVariableLen(Event.var_len);
    end;
  end else begin
    WriteVariableLen(Event.var_len);
  end;
