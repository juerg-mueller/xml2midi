...
begin
  if (smallestNote < 1) or (smallestNote > 4) then
    smallestNote := 2;
  result := DeltaTimeTicks div smallestNote;
end;
...
begin
  ...
  if p = DeltaTimeTicks  div 8 - 1 then
    ...
    result := smallestNote*((p + 2*smallestNote div 3) div smallestNote);
end;
...
begin
  ...
  DeltaTimeTicks := 192;
  beatsPerMin := 120;
  smallestNote := DeltaTimeTicks div 4;
  measureFact := 4;
  measureDiv := 4;
  CDur := 0;
  Minor := false;
